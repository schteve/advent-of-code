...
impl Ord for ItemCount {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        if compare.iter().any(|v| *v == Ordering::Less) {
            Ordering::Less
        } else if compare.iter().any(|v| *v == Ordering::Greater) {
            Ordering::Greater
        } else {
            Ordering::Equal
        }
...
impl Blueprint {
    fn from_str(s: &str) -> Self {
        ...
        let costs = [
            [caps.get(2).unwrap().as_str().parse().unwrap(), 0, 0, 0],
            [caps.get(3).unwrap().as_str().parse().unwrap(), 0, 0, 0],
            [
        ...
        Self {
            id,
            costs: costs.map(ItemCount::from),
        }
    }

    fn most_geodes(&self, time: usize) -> u64 {
        let inventory: ItemCount = [0, 0, 0, 0].into();
        let robots: ItemCount = [1, 0, 0, 0].into();
    ...
    fn quality_level(&self, time: usize) -> u64 {
        self.id * self.most_geodes(time)
    }
}

fn total_quality(blueprints: &[Blueprint], time: usize) -> u64 {
    blueprints.iter().map(|bp| bp.quality_level(time)).sum()
}

fn first_n_product(blueprints: &[Blueprint], time: usize, n: usize) -> u64 {
    blueprints[..n]
        .iter()
        .map(|bp| bp.most_geodes(time))
        .product()
}
...
#[aoc(day19, part1)]
pub fn part1(input: &[Blueprint]) -> u64 {
    let total = total_quality(input, 24);
    assert_eq!(total, 1599);
    total
}

#[aoc(day19, part2)]
pub fn part2(input: &[Blueprint]) -> u64 {
    let total = first_n_product(input, 32, 3);
    assert_eq!(total, 14112);
    total
}
...
Blueprint 2: Each ore robot costs 2 ore. Each clay robot costs 3 ore. Each obsidian robot costs 3 ore and 8 clay. Each geode robot costs 3 ore and 12 obsidian.";
    ...
    #[test]
    fn test_first_n_product() {
        let input = input_generator(EXAMPLE_INPUT);
        let total = first_n_product(&input, 32, 2);
        assert_eq!(total, 56 * 62);
    }
}
