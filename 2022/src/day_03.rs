...
pub fn input_generator(input: &str) -> Vec<(HashSet<char>, HashSet<char>)> {
    input
        ...
        .map(|line| {
            ...
            let left = it.by_ref().take(line.len() / 2).collect();
            let right = it.by_ref().take(line.len() / 2).collect();
...
#[aoc(day3, part1)]
pub fn part1(input: &[(HashSet<char>, HashSet<char>)]) -> u64 {
    let total = total_priority(input);
    assert_eq!(total, 8515);
    total
}

#[aoc(day3, part2)]
pub fn part2(input: &[(HashSet<char>, HashSet<char>)]) -> u64 {
    let total = group_priority(input);
