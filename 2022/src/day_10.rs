...
impl Signal {
    fn emit(instructions: &[Instruction]) -> Self {
        let mut x = 1;
        let data = instructions
            ...
            .flat_map(|inst| match inst {
                Instruction::Add(n) => {
                    let tmp = x;
                    x += *n;
                    iter::repeat(tmp).take(2)
                }
                Instruction::Nop => iter::repeat(x).take(1),
            })
            .collect();
        Self { data }
    }

    fn strength(&self, cycle: NonZeroUsize) -> i64 {
        let cycle = cycle.get();
        self.data[cycle - 1] * cycle as i64
    }

    fn six_strengths(&self) -> i64 {
        [20, 60, 100, 140, 180, 220]
            .into_iter()
            .filter_map(NonZeroUsize::new)
            .map(|i| self.strength(i))
            .sum()
    }

    fn crt_image(&self) -> String {
...
pub fn part1(input: &[Instruction]) -> i64 {
    ...
    let strength = signal.six_strengths();
    assert_eq!(strength, 12840);
    strength
}
...
noop";
    ...
    fn test_six_strengths() {
        ...
        for (cycle, expect) in [
            ...
            let cycle = NonZeroUsize::new(cycle).unwrap();
            assert_eq!(signal.strength(cycle), expect);
        }
        let strength = signal.six_strengths();
