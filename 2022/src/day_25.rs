...
impl Snafu {
    fn from_str(s: &str) -> Self {
        let mut factor = 1;
        let decimal = s.chars().rev().fold(0, |acc, c| {
            let digit = match c {
                '=' => -2,
                '-' => -1,
                '0' => 0,
...
impl Display for Snafu {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let mut dec = self.decimal;
        while dec > 0 {
            let (digit, sub) = match dec % 5 {
                0 => ('0', 0),
                1 => ('1', 1),
                2 => ('2', 2),
                3 => ('=', -2),
                4 => ('-', -1),
                x => panic!("Invalid remainder: {x} from {dec}"),
            };
            digits.push(digit);
            dec -= sub;
            dec /= 5;
...
impl Add<&Self> for Snafu {
    ...
    fn add(self, rhs: &Self) -> Self::Output {
        let decimal = self.as_decimal() + rhs.as_decimal();
        Self { decimal }
    }
}

impl<'a> Sum<&'a Self> for Snafu {
    fn sum<I>(iter: I) -> Self
    where
        I: Iterator<Item = &'a Snafu>,
    {
        iter.fold(Snafu::from_decimal(0), |sum, item| sum + item)
    }
}

fn solve(snafus: &[Snafu]) -> String {
    snafus.iter().sum::<Snafu>().to_string()
}
...
122";
    ...
    fn test_snafu() {
        ...
        for (dec, expected) in table {
            let snafu = Snafu::from_decimal(dec);
            assert_eq!(snafu.to_string(), expected);
        }
    }
