...
fn hash_sum(strings: &[String]) -> u32 {
    strings
        .iter()
        .map(|s| hash(s.as_str()))
        .fold(0, |acc, h| acc + h as u32)
}
...
impl<'a> Hashmap<'a> {
    fn apply_ops(strings: &'a [String]) -> Self {
        let mut me = Self {
            boxes: vec![LensBox::new(); 256],
        };
        for s in strings {
            let (op, label) = if let Some((left, right)) = s.split_once('=') {
                let label = left;
                let focal_length = right.parse().unwrap();
                (Op::Set(focal_length), label)
            ...
            match op {
                Op::Set(focal_length) => {
                    ...
                    if let Some(lens) = me.boxes[box_n as usize]
                        .lenses
                        .iter_mut()
                        .find(|lens| lens.label == label)
                    {
                        ...
                        me.boxes[box_n as usize].lenses.push(lens);
                    }
                }
                Op::Remove => {
                    me.boxes[box_n as usize]
                        .lenses
        ...
        self.boxes
            ...
            .flat_map(|(i, lensbox)| {
                lensbox
                    ...
                    .iter()
                    .enumerate()
                    .map(move |(j, lens)| (i + 1) as u32 * (j + 1) as u32 * lens.focal_length)
            })
            .sum()
    }
}
...
pub fn input_generator(input: &str) -> Vec<String> {
    input.split(',').map(|line| line.to_string()).collect()
}

#[aoc(day15, part1)]
pub fn part1(input: &[String]) -> u32 {
    let value = hash_sum(input);
    assert_eq!(value, 515495);
    value
}

#[aoc(day15, part2)]
pub fn part2(input: &[String]) -> u32 {
    let hashmap = Hashmap::apply_ops(input);
    let value = hashmap.focusing_power();
    assert_eq!(value, 229349);
    value
}
...
mod test {
    ...
    static EXAMPLE_INPUT: &str = "rn=1,cm-,qp=3,cm=2,qp-,pc=4,ot=9,ab=5,pc-,pc=6,ot=7";
